...
class TournamentUiTests(unittest.TestCase):
    ...
    def test_tournament_results_and_history_in_ui(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            tournament = store.create_tournament('Office final', 'single', [p['id'] for p in players])[2][0]
            window = Window(store)
            self.wait(window)
            view = window.tournaments_view
            window.tabs.setCurrentIndex(3)
            self.wait(window)
            iterator = QTreeWidgetItemIterator(view.bracket)
            while iterator.value():
                item = iterator.value()
                if item.data(0, Qt.ItemDataRole.UserRole):
                    view.bracket.setCurrentItem(item)
                    break
                iterator += 1
            self.assertTrue(view.register_button.isEnabled())
            view.score1.setValue(11)
            view.score2.setValue(8)
    ...
    def test_create_tournament_dialog(self):
        from app import Window
        from PyQt6.QtCore import Qt, QTimer
        from PyQt6.QtWidgets import QComboBox, QDialogButtonBox, QLineEdit, QListWidget, QSpinBox
        with tempfile.TemporaryDirectory() as directory:
            store = Store(directory)
            for name in ('Alice', 'Bob', 'Carol', 'Dan'):
                store.save_player(name)
            window = Window(store)
            self.wait(window)
            errors = []
            ...
            def submit():
                ...
                try:
                    dialog.findChild(QLineEdit).setText('Team cup')
                    system = dialog.findChild(QComboBox)
                    system.setCurrentIndex(system.findData('group_double'))
                    members = dialog.findChild(QListWidget)
                    for i in range(members.count()):
                        members.item(i).setCheckState(Qt.CheckState.Checked)
                    dialog.findChild(QSpinBox).setValue(2)
            ...
            window.tournaments_view.create_tournament()
            self.wait(window)
            self.assertEqual(errors, [])
            tournament = store.snapshot()[2][0]
            self.assertEqual((tournament['name'], tournament['groups']), ('Team cup', 2))
            self.assertEqual(window.tournaments_view.selector.currentData(), tournament['id'])
            window.close()
